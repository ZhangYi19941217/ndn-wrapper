...
void __pipe_push(pipe_t* p, const void* elems, size_t count);
...
pipe_t* pipe_new(size_t elem_size, size_t original_limit)
{
    ...
    if(unlikely(p == NULL || buf == NULL))
        return free(p), free(buf), (pipe_t*)NULL;
    *p = (pipe_t) {
        .elem_size  = elem_size,
        .min_cap = cap,
...
size_t pipe_used_size(pipe_t* p)
{
...
void pipe_free(pipe_t* p)
{
    ...
    {
        p->buffer = (free(p->buffer), (char*)NULL);
                                           ...
                                           char* buf)
{
    if(wraps_around(s))
    {
        buf = (char*)offset_memcpy(buf, s.begin, s.bufend - s.begin);
        buf = (char*)offset_memcpy(buf, s.buffer, s.end - s.buffer);
    }
    else
    {
        buf = (char*)offset_memcpy(buf, s.begin, s.end - s.begin);
    }
    return buf;
}
...
static snapshot_t resize_buffer(pipe_t* p, size_t new_size)
{
    ...
    char* new_buf = (char*)malloc(new_size + elem_size);
                               ...
                               )
{
    ...
    if(!wraps_around(s))
    {
        ...
        s.end = (char*)offset_memcpy(s.end, elems, at_end);
    ...
    {
        s.end = wrap_ptr_if_necessary(s.buffer, s.end, s.bufend);
        s.end = (char*)offset_memcpy(s.end, elems, bytes_to_copy);
    }
    s.end = wrap_ptr_if_necessary(s.buffer, s.end, s.bufend);
